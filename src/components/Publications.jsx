...
import { FileText, RotateCcw, Sparkles, CheckCircle2 } from 'lucide-react';
...
export default function Publications() {
  ...
  return (
    <section id="publications" className="relative py-20 lg:py-28 bg-black overflow-hidden">
      ...
      <div className="absolute top-1/2 left-1/3 w-[500px] max-w-full h-[500px] bg-white/[0.015] blur-[150px] rounded-full pointer-events-none" />
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 relative z-10 flex justify-center">
        <motion.div
          initial={{ opacity: 0, y: 40 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          transition={{ duration: 0.8 }}
          className="pub-card-scene w-full max-w-[620px] min-h-[500px] sm:min-h-[420px] cursor-pointer select-none"
          ...
          <div className={`pub-card-inner ${isFlipped ? 'is-flipped' : ''}`}>
            ...
            <div className="pub-card-face pub-card-front bg-[#09090B] border border-[#27272A] p-6 sm:p-8 flex flex-col justify-between shadow-[0_-8px_20px_rgba(0,0,0,0.8)] group hover:border-[#52525B] transition-colors">
              ...
              <div className="flex items-start justify-between flex-wrap gap-2">
                <div className="w-11 h-11 sm:w-12 sm:h-12 rounded-2xl bg-[#18181B] border border-[#27272A] flex items-center justify-center text-[#E8D2A6] shrink-0">
              ...
              <div className="my-auto py-3">
                <h3 className="text-lg sm:text-2xl font-display font-semibold text-white/95 leading-snug tracking-tight mb-2 sm:mb-3">
                  {pub.title}
                </h3>
                ...
                <div className="flex flex-wrap gap-2 sm:gap-2.5">
                  {pub.metrics.map((m, idx) => (
                    <div
                      key={idx}
                      className="px-2.5 sm:px-3 py-1.5 rounded-lg bg-white/[0.03] border border-white/10 flex items-center gap-1.5 sm:gap-2"
                    >
                      <span className="text-xs sm:text-sm font-display font-bold text-white">{m.value}</span>
                      <span className="text-[8.5px] sm:text-[9px] font-mono text-white/40 uppercase tracking-widest">{m.label}</span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="flex items-center justify-between pt-4 border-t border-white/[0.06]">
                <div className="flex items-center gap-1.5 text-xs font-mono text-[#E8D2A6]">
                  ...
                  <span className="tracking-wider uppercase text-[9.5px] sm:text-[10px]">Click or Tap to reveal details</span>
                </div>
